...
var jobCategories = [
  ...
  { id:'build', icon:'???', name:'Строительство' },
...
firebase.database().ref('jobs').on('value', function(snap) {    var jobs = snap.val();
  ...
  + (j.status === 'closed'
          ? (j.selectedWorker === U.huid
            ...
            : alreadyApplied
              ? '<div style="margin-top:10px;font-size:13px;color:#EF4444;text-align:center;padding:8px;background:#FEE2E2;border-radius:8px;">😔 Выбран другой исполнитель</div>'
